...
# total_time表示统计[0, total_time)时间段内的车流
import argparse
import json
import numpy as np
...
parser.add_argument('--total_time', type=int, default=3600, help='the total time of this flow file')
...
# else:
N = int((total_time - 1) / s_interval) + 1
...
time_intervals = [[int(i*s_interval), min(int((i+1)*s_interval), total_time) - 1] for i in range(N)]
...
for vehicle in traffic_flow:
    ...
    for i in range(n_origin):
        ...
        if origin_road in virtual_intersections[i]["roads"]:
            origin_ind = i
